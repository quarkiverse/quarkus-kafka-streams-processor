...
[.description]
--
Allows to automatically insert a timestamp in the CloudEvent when building the outgoing message, if the time was not explicitly set. It is used only if `OutputConfig++#++isCloudEvent()` is `true`.


ifdef::add-copy-button-to-env-var[]
Environment variable: env_var_with_copy_button:+++KAFKASTREAMSPROCESSOR_OUTPUT_CLOUD_EVENTS_INSERT_TIMESTAMP+++[]
endif::add-copy-button-to-env-var[]
ifndef::add-copy-button-to-env-var[]
Environment variable: `+++KAFKASTREAMSPROCESSOR_OUTPUT_CLOUD_EVENTS_INSERT_TIMESTAMP+++`
endif::add-copy-button-to-env-var[]
--
|boolean
|`+++true+++`

...
This extension eases the implementation of event-driven architecture streaming microservices based on https://kafka.apache.org/documentation/streams/[Kafka Streams]’ Processor API.
It avoids Kafka Streams' users some boilerplate code to have a dedicated topology and a proper production readiness (health, observability, error handling).
...
* Global Stores (optional): for stateful use cases, when it's replicated across all instances of the application.
...
The Kafka 2 API `org.apache.kafka.streams.processor.Processor<K, V>` is no longer supported.
...
To used JSON, a processor needs the generated data model as value type.
The data model is read from kafka, or written to kafka, with the data model's JSON textual representation.
...
<3> Same data model value in the _process()_ method.
<4> Similarly, a data model value can be sent down to the output topic.
...
Then the processor should be declared with the custom data model as value type.
The data model is read from kafka, or written to kafka, with a custom serialization format provided by sink and source serializer/deserializer that need to be known at build time.
...
<1> Your Processor is declared with the annotation as for a regular processor.
<2> The handled value type, in this example, is a simple data model, nothing fancy.
<3> Same data model value in the _process()_ method.
<4> Similarly, a data model value can be sent down to the output topic.

And then you inject your custom serde with:
...
include::includes/cloud_events.adoc[leveloffset=+1]
...
| Counter
| The number of messages sent to DLQ.
This metrics differs from the `kafkastreamsprocessor.processor.errors` in that the deserialization errors are taken into account.
If no DLQ strategy is chosen, then this metric will be 0.
Messages too large for the DLQ will be counted both in this metrics and in `kafkastreamsprocessor.global.dlq.sent`.
...
WARNING: Starting asynchronous processing in a Kafka Streams microservice would lead to loss of messages: acknowledgment of the incoming messages will be de-synchronized from the production of messages once outgoing messages have been processed.
...
It's up to the https://smallrye.io/smallrye-reactive-messaging/3.21.0/kafka/receiving-kafka-records/#acknowledgement[acknowledgment policy] to commit incoming messages when it has been processed concurrently.
...
include::includes/stateful_eda.adoc[leveloffset=1]
...
`ProducerInterceptor` are called after the key and value serialization, at the difference with xref:_output_record_interceptor[`OutputRecordInterceptor`] which are called before.

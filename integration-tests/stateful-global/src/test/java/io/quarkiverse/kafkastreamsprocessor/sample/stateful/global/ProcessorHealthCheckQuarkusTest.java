...
public class ProcessorHealthCheckQuarkusTest {
    ...
    public void testHealthChecksDuringLifecycle() throws Exception {
        String globalStoreTopic = kStreamsProcessorConfig.globalStores().get("store-data").topic();
        String globalStoreTopicCapital = kStreamsProcessorConfig.globalStores().get("store-data-capital").topic();
        String inputTopic = kStreamsProcessorConfig.input().topic().get();
        String outputTopic = kStreamsProcessorConfig.output().topic().get();
        ...
        checkReadiness().responseCode(200)
                .isUp(true)
                .topicsCheck(true,
                        String.join(",", inputTopic, outputTopic, globalStoreTopic, globalStoreTopicCapital),
                        null)
                .stateCheck(true, "RUNNING");

        try (AdminClient admin = AdminClient
                .create(Map.of(AdminClientConfig.BOOTSTRAP_SERVERS_CONFIG, kafkaBootstrapServers))) {
            admin.deleteTopics(Collections.singletonList(globalStoreTopicCapital)).all().get(
                    10L,
                    TimeUnit.SECONDS);
        }
        ...
        checkReadiness().responseCode(503)
                .isUp(false)
                .topicsCheck(false,
                        String.join(",", inputTopic, outputTopic, globalStoreTopic),
                        globalStoreTopicCapital)
                .stateCheck(true, "RUNNING");
